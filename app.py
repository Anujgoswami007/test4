...
verifier = pipeline("question-answering", model="deepset/roberta-base-squad2")
...
sample_corpus = [
    ...
    "Platform Focus: Cross-border payment platform for businesses, facilitating international payments, currency conversion, and transfers to Indian bank accounts.",
    "Services Offered: Virtual accounts, invoicing, recurring payments, and analytics.",
    "Security Measures: Industry-standard encryption, two-factor authentication, and compliance with AML (Anti-Money Laundering) and KYC (Know Your Customer) regulations.",
    "Regulation: Regulated by RBI (Reserve Bank of India) guidelines.",
    "Cross-border payments with currency conversion.",
    "Local collection of foreign payments.",
    "Creation of virtual accounts in various currencies.",
    "Invoice generation, recurring payments, and payment tracking.",
    "Reporting, analytics, and transaction reconciliation.",
    "Platform Fee: 0.99% (minimum $3) for INR withdrawals in USD, GBP, EUR, and CAD; 1.25% for other currencies.",
    "High-Value Withdrawals: $30 fee + 0.20% for amounts over $25,000.",
    "No subscription fees; no hidden charges are mentioned, but users should review the terms carefully.",
    "Maximum transaction limit is $25,000 (25 lakhs INR) via PA-CB route; higher amounts are processed via SWIFT.",
    "Funds are accessible immediately in the virtual account and can be quickly transferred to Indian bank accounts after conversion.",
    "Upcoming Features: Mass payment support and integration with accounting software.",
    "E-commerce: Supports cross-border e-commerce transactions.",
    "Availability: 24/7 via live chat and email (English only).",
    "Support Materials: Knowledge base and FAQ section available on the website.",
    "AML and KYC Compliance: Adheres to RBI guidelines for AML and KYC; no legal or tax guidance is provided.",
    "Typically takes 2 days for international transfers, with real-time tracking available.",
    "Supports multiple virtual accounts in different currencies, though only business accounts are available (no personal accounts).",
    "KYC verification is required, but no minimum deposit is necessary."
]
...
def retrieve_documents(query: str) -> Optional[Dict[str, Any]]:
    ...
    if response['matches']:
        ...
        # Rerank results
        pairs = [[query, match['answer']] for match in matches]
        rerank_scores = reranker.predict(pairs)
        for idx, score in enumerate(rerank_scores):
            matches[idx]['rerank_score'] = score
        
        matches.sort(key=lambda x: x['rerank_score'], reverse=True)
        best_match = matches[0]
        
        # Add confidence level based on rerank score
        confidence_level = get_confidence_level(best_match['rerank_score'])
        best_match['confidence'] = confidence_level
...
def check_semantic_similarity(query: str, answer: str) -> float:
    ...
    similarity = util.pytorch_cos_sim(query_embedding, answer_embedding)
    return similarity.item()

def verify_answer(query: str, context: str, generated_answer: str) -> bool:
    result = verifier(question=query, context=context)
    if result['score'] > 0.8 and result['answer'] in generated_answer:
        return True
    return False
...
def chatbot():
    if request.method == 'GET':
        ...
        if not query:
            return jsonify({
                "error": "No query provided in GET parameters. Please use ?query=your_question"
            }), 400
    else:  # POST request
        data = request.json
        query = data.get('query')
        if not query:
            return jsonify({"error": "No query provided in POST body"}), 400

    # Process the query (same logic for both GET and POST)
    ...
    if document:
        ...
        return jsonify({
            ...
            "query": query,  # Echo back the query for verification
            "method": request.method  # Include the HTTP method used
        })
